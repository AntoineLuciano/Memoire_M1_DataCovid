...
df <- CSVtoR(var)
names_col = c("Age","PCSI","Sexe","Emp","C10","A1","A2","C6_0","C6_1","C6_2","C6_3","Vague")
...
reg = glm(formula = Infecte ~ PCSI + HorsDomicile + Sexe + Age + Diplome + ProfSante, family=binomial(link=logit),data= df)
summary(reg)

tbl_regression(reg, exponentiate = TRUE)

#FIGURE 25
table_Infecte<-t(data.frame(Nombre = as.character(unclass(table(df$Infecte))), 
                            Frequence = paste(as.character(round(unclass(table(df$Infecte))/sum(!is.na(df$Infecte))*100,3)),"%")))
