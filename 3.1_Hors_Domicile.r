...
var <- c("AGE2", "PCSI", "SEXE", "EMP", "C10", "C6_0", "C6_1", "C6_2", "C6_3",
         "B2_0", "B2_1", "B2_2", "B2_3", "B2_4", "B2_5", "B2_6", "B2_7", "B2_8")
df <- CSVtoR_avec_Rev(var)
names_col <- c("Age", "PCSI", "Sexe", "Emp", "C10", "C6_0", "C6_1", "C6_2", "C6_3", 
               "lavageMain", "gelHydroAlcoolique", "tousserCoude", "mouchoireUnique",
               "distance1M", "eviteSerrerMainEmbrasser", "eviteRegroupements", 
               "resterChezSoi", "porterMasque", "Vague", "Revenus")
...
df$groupe3 <- rowSums(df[, c("lavageMain1", "tousserCoude1", "mouchoireUnique1",
                             "distance1M1", "eviteSerrerMainEmbrasser1",
                             "eviteRegroupements1")])
...
df$groupe1[df$porterMasque == 7 | df$lavageMain == 7 | df$gelHydroAlcoolique == 7
           | df$tousserCoude == 7 | df$mouchoireUnique == 7 | df$distance1M == 7 
           | df$eviteSerrerMainEmbrasser == 7 | df$eviteRegroupements == 7 
           | df$resterChezSoi == 7] <- NA
df$groupe2[df$porterMasque == 7 | df$lavageMain == 7 | df$gelHydroAlcoolique == 7 
           | df$tousserCoude == 7 | df$mouchoireUnique == 7 | df$distance1M == 7 
           | df$eviteSerrerMainEmbrasser == 7 | df$eviteRegroupements == 7 
           | df$resterChezSoi == 7] <- NA
df$groupe3[df$porterMasque == 7 | df$lavageMain == 7 | df$gelHydroAlcoolique == 7 
           | df$tousserCoude == 7 | df$mouchoireUnique == 7 | df$distance1M == 7 
           | df$eviteSerrerMainEmbrasser == 7 | df$eviteRegroupements == 7 
           | df$resterChezSoi == 7] <- NA
...
df$PCSI <- mgsub(df$PCSI, pattern = c("1", "2", "3", "4", "5", "6", "7", "8"),
                 replacement = metier_cat)
...
df$Diplome <- factor(df$Diplome, levels = c("Sans Diplome", "- de BAC",
                                            "BAC à BAC+2", "BAC+3 ou +", "Autre"))

# Variable Revenus
rev_seuil <- c("1000€ ou -", "1000€ ou -", "1000€ ou -", "1000-2000€",
               "1000-2000€", "2000-3000€", "3000€ ou +", NA)
...
df$Revenus <- factor(df$Revenus, levels = c("1000€ ou -", "1000-2000€",
                                            "2000-3000€", "3000€ ou +"))


# HD expliqué par les gestes barrières
reg_GB <- glm(formula = HorsDomicile ~ groupe1 + groupe2 + groupe3, 
              family = binomial(link = "logit"), data = df)

tbl_regression(reg_GB, exponentiate = TRUE)

# HD expliqué par PCSI Age Diplome et ProfSANTE
reg <- glm(formula = HorsDomicile ~ PCSI + Sexe + Age + Diplome + ProfSante, 
           family = binomial(link = logit), data = df)

tbl_regression(reg, exponentiate = TRUE)


# Figure 23 : Nombre et fréquence des personnes travaillant Hors Domicile dans notre échantillon
table_HD <- t(data.frame(
  Nombre = as.character(unclass(table(df$HorsDomicile))),
  Frequence = paste(as.character(round(unclass(table(df$HorsDomicile)) 
                                    / sum(!is.na(df$HorsDomicile)) * 100, 3)), "%")
))
colnames(table_HD) <- c("Travaille chez soi", "Travaille à l'extérieur")

View(table_HD)

# Figure 24 : Évolution de la proportion des travailleurs Hors Domicile en fonction du temps
HD <- numeric(8)

for (i in 1:8) {
  HD[i] <- sum(as.integer(df$HorsDomicile[df$Vague == i & 
                !is.na(df$HorsDomicile)])) / length(!is.na(df$HorsDomicile)) * 100
}

date <- as.Date(c("2020-04-07", "2020-04-15", "2020-04-22", "2020-04-28", 
                  "2020-05-05", "2020-05-12", "2020-05-26", "2020-06-05"))

graph <- data.frame(date)
graph <- cbind(graph, HD)

g <- qplot(date, HD, data = graph, geom = c("point", "path"), 
           main = "Evolution de la proportion de travailleurs Hors Domicile",
           ylab = "Proportion de travailleurs Hors Domicile (en %)", xlab = "Date")
g <- g + geom_vline(xintercept = as.Date("2020-05-11"), col = "red")
g + geom_text(x = as.Date("2020-04-27"), y = 10.9, 
              label = "Déconfinement du 11 mai 2020", color = "red",
              size = 3.7, fontface = 1)
